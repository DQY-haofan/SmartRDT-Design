...
class MetricsCalculator:
    ...
    def select_representative_solutions(pareto_df):
        ...
        extremes = {'f1_total_cost_USD': ('Min Cost', 'min'), 'detection_recall': ('Max Recall', 'max'),
                   'f3_latency_seconds': ('Min Latency', 'min'), 'f5_carbon_emissions_kgCO2e_year': ('Min Carbon', 'min')}
        ...
        if len(pareto_df) > 0 and len(obj_cols) > 0:
            ...
            for c in obj_cols:
                ...
                if rng > 0:
                    norm[c] = (norm[c] - norm[c].min()) / rng if c != 'detection_recall' else 1 - (norm[c] - norm[c].min()) / rng
...
class Visualizer:
    ...
    def plot_convergence(self, history, save_name='convergence.pdf'):
        ...
        if conv.get('hv'):
            ...
            axes[0].set_xlabel('Generation'); axes[0].set_ylabel('Hypervolume')
            axes[0].set_title('(a) HV Convergence', fontsize=10)
        
        if conv.get('n_nds'):
            axes[1].plot(gens, conv['n_nds'], '-s', color=COLORS['grid'], markersize=3)
            axes[1].set_xlabel('Generation'); axes[1].set_ylabel('Pareto Size')
    ...
    def plot_pareto_2d_main(self, df, baselines=None, save_name='pareto_2d_main.pdf'):
        ...
        ax.set_xlabel('Cost (M$)'); ax.set_ylabel('Recall')
    ...
    def plot_pareto_3d(self, df, save_name='pareto_3d.pdf'):
        ...
        ax.set_xlabel('Cost (M$)'); ax.set_ylabel('Recall'); ax.set_zlabel('Carbon (t)')
        ax.view_init(20, 45)
        self._save(fig, save_name)
    ...
    def plot_method_summary(self, pareto_df, baselines, save_name='method_summary.pdf'):
        ...
        axes[0,0].set_ylabel('Feasible'); axes[0,0].set_title('(a) Solutions', fontsize=10)
        axes[0,0].tick_params(axis='x', rotation=45)
        
        # (b) Cost
        vals = [metrics[m]['cost'] for m in methods]
        axes[0,1].bar(methods, vals, color=colors, alpha=0.8)
        axes[0,1].set_ylabel('Min Cost (M$)'); axes[0,1].set_title('(b) Cost', fontsize=10)
        axes[0,1].tick_params(axis='x', rotation=45)
        
        # (c) Recall
        vals = [metrics[m]['recall'] for m in methods]
        axes[1,0].bar(methods, vals, color=colors, alpha=0.8)
        axes[1,0].set_ylabel('Max Recall'); axes[1,0].set_title('(c) Recall', fontsize=10)
        ...
        axes[1,1].set_xlabel('Cost (M$)'); axes[1,1].set_ylabel('Recall')
    ...
    def _gen_summary_json(self, pareto_df, baselines, out_dir):
        summary = {
            ...
            'pareto': {'count': len(pareto_df),
                      'cost_range': [float(pareto_df['f1_total_cost_USD'].min()), float(pareto_df['f1_total_cost_USD'].max())],
                      'recall_range': [float(pareto_df['detection_recall'].min()), float(pareto_df['detection_recall'].max())]},
            'baselines': {n: {'total': len(d), 'feasible': int(d['is_feasible'].sum()) if 'is_feasible' in d.columns else len(d)}
