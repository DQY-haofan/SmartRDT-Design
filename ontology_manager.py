...
class OntologyManager:
    ...
    def populate_from_csv_files(self, data_dir: str = 'data'):
        """从CSV文件填充本体"""
        data_path = Path(data_dir)
        logger.info("从CSV文件填充本体...")
        
        # 传感器
        sensors_file = data_path / 'sensors_data.txt'
        if sensors_file.exists():
            self._load_sensors(sensors_file)
        
        # 算法
        algorithms_file = data_path / 'algorithms_data.txt'
        if algorithms_file.exists():
            self._load_algorithms(algorithms_file)
        
        # 基础设施
        infra_file = data_path / 'infrastructure_data.txt'
        if infra_file.exists():
            self._load_infrastructure(infra_file)
        
        # 成本效益参数
        cost_file = data_path / 'cost_effectiveness_data.txt'
        if cost_file.exists():
