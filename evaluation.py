...
class EnhancedFitnessEvaluatorV3:
    ...
    def _calculate_total_cost(self, config: Dict) -> float:
        ...
        # FOS sensor special handling
        if sensor_type == 'FOS':
            sensor_spacing_km = self.config.fos_sensor_spacing_km
            sensors_needed = road_length / sensor_spacing_km
            total_sensor_initial = sensor_initial * sensors_needed + 5000 * sensors_needed
        else:
            total_sensor_initial = sensor_initial
        ...
        if coverage_km_day > 0:  # Mobile sensor
            ...
            if sensor_type == 'FOS':
                sensors_needed = road_length / self.config.fos_sensor_spacing_km
                sensor_annual_opex = 0.5 * sensors_needed * 365
            else:
                sensor_annual_opex = sensor_daily_cost * 365
        ...
        if coverage_km_day > 0:
            ...
            maintenance_days = 10 if sensor_type == 'FOS' else 20
        ...
        if annotation_mult > 0:
            ...
            # Images per year estimate
            if 'Camera' in sensor_name:
                annual_images = 100 * road_length * inspections_per_year
            else:
                annual_images = 10000 * inspections_per_year
            annotation_annual = base_annotation_cost * annual_images * annotation_mult * 0.3  # Active learning
        else:
            annotation_annual = 0
        ...
        # Low detection threshold → more false positives → more verification cost
        ...
        if getattr(self.config, 'apply_seasonal_adjustments', True):
            total_annual *= 1.075  # 7.5% winter adjustment
    ...
    def _calculate_environmental_impact(self, config: Dict) -> float:
        ...
        electricity_emissions = (sensor_kwh + comm_kwh + storage_kwh + compute_kwh) * carbon_intensity * deploy_carbon_factor
